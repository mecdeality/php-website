...
if(isset($_POST['edit'])){
    $id = $_POST['id'];
    $cat_id = $_POST['cat_id'];
    header("Location:edit_news.php?id=".$id."&cat_id=".$cat_id);
}
...
if(isset($_POST['delete'])){
    $id = $_POST['id'];
    $run_delete = mysqli_query($conn, "DELETE FROM news WHERE id = '$id'");
                ...
                <div class="news-left-back">
                    ...
                    <div class="d-flex flex-wrap">
                        <?php while($news = mysqli_fetch_assoc($run)){ ?>
                            <a href="news.php?id=<?=$news['id'].'&user_id='.$user_id?>">
                                <div class="news">
                                    <img src="<?=$news['img1']?>" class="img-responsive img-resize">
                                    <div class="edit">
                                        <form method="post">
                                            <input type="hidden" name="id" value="<?=$news['id']?>">
                                            <input type="hidden" name="cat_id" value="<?=$news['categorie_id']?>">
                                            <input id="submitButton" type="submit" name="delete" value="&#xf00d">
                                            <input id="submitButton" type="submit" name="edit" value="&#xf044">
                                        </form>
                                    </div>
                    ...
                    <div class="pagination">
                        <a id="non" href="index_admin.php?page=<?=$page-1?>">&laquo;</a>
                        <?php for($i = 1; $i <= $page_num; $i++) { ?>
                            <a class="page" id="<?php $page == $i ? print("active") : print($i); ?>"
                               href="index_admin.php?page=<?= $i ?>"><?= $i ?></a>
                            <?php } ?>
                        <a id="non" href="index_admin.php?page=<?=$page+1?>">&raquo;</a>
                    </div>
                    <?php } ?>
                </div>
            </div>
